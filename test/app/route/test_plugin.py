...
def test_plugin_compute(mocked_client, default_plugin, general_uuid, default_aoi):
    with patch('api_gateway.app.route.plugin.uuid.uuid4', return_value=general_uuid):
        response = mocked_client.post('/plugin/test_plugin', json={'aoi': default_aoi, 'params': dict()})
